  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--cb-bg)', color: 'var(--cb-text-primary)', overflowX: 'hidden' }}>
      ...
      <section style={{ maxWidth: 900, margin: '80px auto 60px auto', textAlign: 'center', padding: '0 var(--cb-space-4)' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(99, 102, 241, 0.1)', color: 'var(--cb-primary)', padding: '6px 14px', borderRadius: 20, fontSize: 13, fontWeight: 600, marginBottom: 24 }}>
        ...
        <h1 style={{ fontSize: 'clamp(40px, 6vw, 64px)', fontWeight: 800, marginBottom: 24, lineHeight: 1.1, letterSpacing: '-1.5px' }}>
          Predictive mental health for the <span style={{ background: 'var(--cb-primary-gradient)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>digital generation.</span>
        </h1>
      ...
      <section style={{ maxWidth: 1000, margin: '0 auto 80px auto', padding: '0 var(--cb-space-4)' }}>
        <div className="glass-card" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 24, textAlign: 'center', padding: '32px' }}>
          <div>
            <div style={{ fontSize: 36, fontWeight: 800, color: 'var(--cb-primary)', marginBottom: 4 }}>99.4%</div>
            <div style={{ fontSize: 14, color: 'var(--cb-text-secondary)' }}>Crisis Early Detection Rate</div>
          </div>
          <div>
            <div style={{ fontSize: 36, fontWeight: 800, color: 'var(--cb-success)', marginBottom: 4 }}>&lt; 30s</div>
            <div style={{ fontSize: 14, color: 'var(--cb-text-secondary)' }}>Parent Escalation Speed</div>
          </div>
          <div>
            <div style={{ fontSize: 36, fontWeight: 800, color: 'var(--cb-text-primary)', marginBottom: 4 }}>100%</div>
            <div style={{ fontSize: 14, color: 'var(--cb-text-secondary)' }}>Privacy & Encryption First</div>
          </div>
        </div>
      </section>
      ...
      <section style={{ maxWidth: 1100, margin: '0 auto 100px auto', padding: '0 var(--cb-space-4)' }}>
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          <h2 style={{ fontSize: 32, fontWeight: 700, marginBottom: 12, letterSpacing: '-0.5px' }}>Engineered for Total Peace of Mind</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 24 }}>
          {[
            {
              title: "SMS-Native & Frictionless",
              desc: "No apps to download or bypass. Chatterbot meets teens where they already text, ensuring high daily engagement without friction."
            },
            {
              title: "Predictive Sentiment Modeling",
              desc: "Our algorithms map emotional trendlines over time, flagging subtle behavioral drops days before high-risk events occur."
            },
            {
              title: "Automated 988 Escalation",
              desc: "If severe distress is identified, the system immediately provides crisis intervention frameworks to the teen while notifying guardians."
            }
          ].map((f, i) => (
            <div key={i} className="glass-card" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ width: 40, height: 40, borderRadius: 'var(--cb-radius-md)', background: 'rgba(99, 102, 241, 0.1)', color: 'var(--cb-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>
                0{i+1}
              </div>
