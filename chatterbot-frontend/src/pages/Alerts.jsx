...
function urgencyFrom(alert) {
  if (alert.status === 'triggered' || alert.severity === 'critical')
    return 'Immediate'
  if (alert.status === 'parent_notified' || alert.severity === 'high')
    return 'High'
...
function confidenceFrom(alert) {
  return typeof alert.confidence === 'number'
    ? `${Math.round(alert.confidence * 100)}% system estimate`
    : 'Not available'
}
...
export default function Alerts() {
  ...
  const updateOwner = async (alertId, assignedTo) => {
    try {
      const result = await api.updateAlertWorkflow(alertId, {
        assigned_to: assignedTo,
      })
      setAlerts((current) =>
        current.map((item) => (item.id === alertId ? result.alert : item))
      )
      setBanner('Alert owner saved.')
    } catch (error) {
      setBanner(error?.data?.error || 'Could not save alert owner.')
    }
  }
  ...
  const filtered = useMemo(() => {
    return alerts.filter((alert) => {
      ...
      if (severityFilter !== 'all' && alert.severity !== severityFilter)
        return false
      const target =
        `${alert.teen_name || ''} ${alert.context_summary || ''} ${alert.status || ''}`.toLowerCase()
  ...
  const applyBulkAction = async (action) => {
    ...
    try {
      ...
      setBanner(
        `${selected.length} alert${selected.length > 1 ? 's' : ''} ${action === 'acknowledge' ? 'acknowledged' : 'resolved'}.`
      )
      setSelected([])
      setBulkNotes('')
    } catch (error) {
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ marginBottom: 'var(--cb-space-5)' }}>
        ...
        <p style={{ color: 'var(--cb-text-secondary)', fontSize: 15 }}>
          Triage by urgency, assign owners, apply bulk workflows, and run
      ...
      >
        <div
          style={{
            display: 'grid',
            gap: 10,
            gridTemplateColumns: '1.5fr repeat(3, minmax(120px, 1fr))',
          }}
        >
          <input
            value={search}
        ...
        >
          ...
          />
          <button onClick={saveCurrentView} style={buttonGhostStyle}>
            Save view
          </button>
          {savedViews.map((view) => (
            <button
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <button
            ...
            onClick={() => applyBulkAction('acknowledge')}
            style={buttonPrimaryStyle}
          >
            {bulkBusy
              ? 'Processing...'
              : `Acknowledge selected (${selected.length})`}
          </button>
          <button
            disabled={selected.length === 0 || bulkBusy}
            onClick={() => applyBulkAction('resolve')}
            style={buttonPositiveStyle}
          >
            {bulkBusy
              ? 'Processing...'
        ...
        >
          <div
            style={{
              ...
              color: 'var(--cb-text-tertiary)',
              display: 'grid',
              gridTemplateColumns: '40px 1.4fr 0.8fr 0.8fr 0.8fr 0.8fr',
            }}
          >
            <span />
            <span>Alert</span>
          ...
          {filtered.length === 0 ? (
            ...
            >
              No alerts match this triage view.
            </div>
          ) : (
            filtered.map((alert) => (
              <div
              ...
              >
                ...
                <span style={statusBadgeStyle(alert.status)}>
                ...
                <select
                  ...
                  onChange={(event) =>
                    updateOwner(alert.id, event.target.value)
                  }
                  style={controlStyle}
                >
        ...
        >
          ...
          >
            ...
            <select
              ...
              style={{ ...controlStyle, width: '100%', marginBottom: 10 }}
            >
              <option value="checkin_call">
                Immediate guardian check-in call
              </option>
              <option value="school_escalation">
                School counselor escalation
              </option>
              <option value="wellness_plan">
                72-hour wellness follow-up plan
              </option>
            </select>
            ...
            >
              Suggested step:{' '}
              {playbook === 'checkin_call'
                ? 'Call guardian/teen now and capture acknowledgement notes.'
                : playbook === 'school_escalation'
                  ? 'Prepare counselor brief with privacy-safe summary and status timeline.'
                  : 'Schedule two check-ins over 72h and monitor status transitions.'}
            </p>
          </div>
