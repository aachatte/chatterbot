...
function getStoredWidgets() {
  try {
    return {
      ...DEFAULT_WIDGETS,
      ...JSON.parse(localStorage.getItem(WIDGET_STORAGE_KEY) || '{}'),
    }
  } catch {
...
export default function Dashboard() {
  ...
  const [guardianRole, setGuardianRole] = useState(
    localStorage.getItem(ROLE_STORAGE_KEY) || 'primary_guardian'
  )
  ...
  useEffect(() => {
    Promise.all([api.getOverview(), api.getTeens()])
      ...
      .catch((e) =>
        setError(
          e.data?.error || 'Dashboard data is unavailable. Please try again.'
        )
      )
  }, [navigate])
  ...
  const commandCenter = useMemo(() => {
    ...
    const pendingEnrollment = teensData.filter(
      (teen) =>
        !teen.consent_verified || teen.phone_verification_status !== 'verified'
    )
    ...
    const trends = [
      {
        ...
        detail: 'Current alerts requiring review',
      },
      {
        label: 'Messages · 7 days',
        value: `${summary.total_messages_7d || 0}`,
        detail: 'Recent message activity',
      },
      {
        label: 'Total safety alerts',
        value: `${summary.total_crisis_alerts || 0}`,
        detail: 'Recorded alerts across teen profiles',
      },
    ]
    return {
      summary,
    ...
    return (
      <div className="db-root">
        <div
          ...
          role="alert"
          style={{ padding: 24, color: 'var(--cb-danger)' }}
        >
          {error}
        </div>
      </div>
  ...
  const greetingName =
    user?.first_name || overview?.parent?.first_name || 'Guardian'
  const roleCopy =
  ...
  return (
    <div className="db-root">
      <div className="db-header">
        <div>
          ...
          <p className="db-header__sub">
            Good to see you, {greetingName}. {roleCopy}
          </p>
        </div>

        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <PointsBadge />
          <Link to="/dashboard/teens" className="btn btn--sm btn--primary-sm">
            Manage teens
          </Link>
        </div>
      </div>
      ...
      <section
        className="db-card"
        style={{ marginBottom: 'var(--cb-space-4)' }}
      >
      ...
      <div className="db-stats">
        ...
        <div
          className={`db-stat-card${summary.active_alerts > 0 ? ' db-stat-card--danger' : ''}`}
        >
          <div className="db-stat-card__label">Active alerts</div>
          <div className="db-stat-card__value">{summary.active_alerts}</div>
          <div className="db-stat-card__sub">
            {summary.active_alerts > 0 ? 'requires triage' : 'all clear'}
          </div>
        </div>
        <div className="db-stat-card">
          <div className="db-stat-card__label">Pending enrollments</div>
          <div className="db-stat-card__value">
            {commandCenter.pendingEnrollmentCount}
          </div>
          <div className="db-stat-card__sub">
            consent/verification to complete
          </div>
        </div>
      </div>
      ...
      {widgets.briefing && (
        <section className="db-card" style={{ marginTop: 'var(--cb-space-4)' }}>
          ...
          >
            <Link to="/dashboard/alerts" className="db-card__action">
              Open triage workspace →
            </Link>
            <Link to="/dashboard/onboarding" className="db-card__action">
          ...
          {queue.length === 0 ? (
            ...
            queue.slice(0, 6).map((item) => (
              <Link
                ...
                to={item.action}
                className="db-alert-item"
                style={{ marginBottom: 8 }}
              >
                <span className="db-alert-badge">
                  {item.type === 'alert' ? 'TRIAGE' : 'SETUP'}
                </span>
      ...
      <div className="db-grid" style={{ marginTop: 'var(--cb-space-4)' }}>
        {widgets.teens && (
          <section className="db-card" aria-labelledby="teens-heading">
            <div className="db-card__header">
              <h2 id="teens-heading" className="db-card__title">
                Teen journey snapshots
              </h2>
              <Link to="/dashboard/teens" className="db-card__action">
                Manage →
              </Link>
            </div>
            {overview.teens.length === 0 ? (
              ...
              overview.teens.map((teen) => (
                <Link
                  ...
                  className="db-teen-item"
                >
                  <div className="db-teen-avatar">
                    <Initials name={teen.first_name} />
                  </div>
                  <div>
                    <div className="db-teen-name">{teen.first_name}</div>
                    <div className="db-teen-meta">
                      {teen.message_count_7d} messages · {teen.mood_label}{' '}
                      signal
                    </div>
                  </div>
        ...
        {widgets.alerts && (
          <section className="db-card" aria-labelledby="alerts-heading">
            <div className="db-card__header">
              <h2 id="alerts-heading" className="db-card__title">
                Recent alert stream
              </h2>
              <Link to="/dashboard/alerts" className="db-card__action">
                View all →
            ...
            {alerts.length === 0 ? (
              ...
              alerts.map((alert) => (
                ...
                >
                  ...
                  <div className="db-alert-severity">
                    {alert.status.replace('_', ' ')} · {alert.teen_name}
                  </div>
